    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between mt-6 md:mt-8 gap-5 md:gap-6 relative z-10">
        ...
        <div className="flex gap-2 md:gap-3 flex-wrap justify-center md:justify-end w-full md:w-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <SocialBtn type="website" url={siteConfig.social?.website} />
...
function SocialBtn({ type, url, onClick }: { type: string, url?: string, onClick?: () => void }) {
  const getIcon = () => {
    switch (type) {
      ...
      case 'website': return <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="9" strokeWidth="2"/><path strokeWidth="2" d="M3 12h18M12 3c2.5 2.5 3.8 5.5 3.8 9S14.5 18.5 12 21M12 3C9.5 5.5 8.2 8.5 8.2 12S9.5 18.5 12 21"/></svg>;
      case 'bilibili': return <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><rect x="3" y="6" width="18" height="14" rx="3" strokeWidth="2"/><path strokeWidth="2" strokeLinecap="round" d="m8 3 2 3m6-3-2 3m-6 7h.01M16 13h.01"/></svg>;
