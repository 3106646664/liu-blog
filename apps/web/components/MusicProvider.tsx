...
function parseLrc(lrcText: string) {
  ...
  for (let line of lines) {
    ...
    if (matches.length > 0) {
      ...
      if (cleanText) {
        const hasInlineWordTiming = matches.length > 1 && matches.slice(1).some((match) => {
          const previous = matches[matches.indexOf(match) - 1];
          return line.slice((previous.index || 0) + previous[0].length, match.index).trim().length > 0;
        });
        for (const match of hasInlineWordTiming ? matches.slice(0, 1) : matches) {
...
const MUSIC_CACHE_KEY = 'xinghui-blog-qq-playlist-v1';
const MUSIC_CACHE_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
const MUSIC_FETCH_ATTEMPTS = 3;

const wait = (milliseconds: number) =>
  new Promise((resolve) => setTimeout(resolve, milliseconds));

function normalizePlaylist(rawResults: any[]) {
  return rawResults
...
export function MusicProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    let isMounted = true;
    let hasCachedPlaylist = false;

    try {
      const cachedText = window.localStorage.getItem(MUSIC_CACHE_KEY);
      if (cachedText) {
        const cached = JSON.parse(cachedText);
        if (
          Array.isArray(cached.playlist) &&
          cached.playlist.length > 0 &&
          Date.now() - Number(cached.cachedAt) < MUSIC_CACHE_MAX_AGE_MS
        ) {
          hasCachedPlaylist = true;
    ...
    const fetchMusicData = async () => {
      for (let attempt = 0; attempt < MUSIC_FETCH_ATTEMPTS; attempt += 1) {
        try {
          const res = await fetch('/api/music', {
            cache: 'no-store',
            signal: AbortSignal.timeout(12000)
          });
          if (!res.ok) throw new Error(`music_http_${res.status}`);

          const rawResults = await res.json();
          const mergedPlaylist = normalizePlaylist(Array.isArray(rawResults) ? rawResults : []);
          if (mergedPlaylist.length === 0) throw new Error('empty_playlist');

          if (isMounted) {
            setPlaylist(mergedPlaylist);
            setIsLoading(false);
            try {
              window.localStorage.setItem(MUSIC_CACHE_KEY, JSON.stringify({
                cachedAt: Date.now(),
          ...
          if (attempt < MUSIC_FETCH_ATTEMPTS - 1) {
      ...
      if (isMounted) {
        if (!hasCachedPlaylist) setCurrentLyric("网络初始化失败");
        setIsLoading(false);
      }
    };
