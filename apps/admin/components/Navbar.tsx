...
export default function Navbar() {
  ...
  useEffect(() => {
    const fetchPath = async () => {
      try {
        ...
        const res = await fetch(`/cms-api/deploy/config`);
        if (res.ok) {
  ...
  // 🌟 监控增强版更新逻辑
  const applyPendingOperations = async (): Promise<boolean> => {
      ...
      try {
        ...
        showToast("✅ 任务已全部写入服务器工作区！", "success");
        clearOperations();
  ...
  const handleSyncBlogClick = () => {
    if (!targetBlogPath) {
      showToast("服务器工作区尚未配置，请检查部署设置。", "error");
      setIsOpBoxOpen(false);
      return;
    }
    setIsOpBoxOpen(false);
  ...
  const executeServerPublish = async () => {
    ...
    try {
      const operationsApplied = await applyPendingOperations();
      if (!operationsApplied) return;

      showToast("正在整理服务器工作区并准备构建主站...", "info");

      const res = await fetch(`/cms-api/sync/execute`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data.success) {
        showToast("服务器工作区已同步，正在启动主站构建...", "info");

        const publishRes = await fetch(`/cms-api/deploy/publish-local`, {
          method: 'POST'
        });
        const publishData = await publishRes.json();
        if (publishData.success) {
          showToast(publishData.message, "success");
        } else {
          showToast(`工作区同步成功，但构建启动失败：${publishData.message}`, "error");
        }
      } else {
        showToast(`服务器工作区同步失败：${data.message}`, "error");
      }
    } catch (error) {
      showToast("无法连接服务器管理后端", "error");
    }
  };
                ...
                {isOpBoxOpen && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 mt-3 w-80 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-4 z-50 cursor-default">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button onClick={handleOpenMainSite} className="py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-black hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                        ↗ 查看主站
                      </button>
                      <button onClick={handleSyncBlogClick} className="py-2.5 rounded-xl bg-indigo-500 text-white text-xs font-black shadow-lg shadow-indigo-500/30 hover:bg-indigo-600 transition-colors">
                        🚀 发布到主站
                      </button>
                    </div>
                  </motion.div>
        ...
        {syncModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[40px] shadow-2xl border border-white/50 p-10 text-center">
              ...
              <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">发布到主站</h3>
              <p className="text-sm text-slate-500 mb-8 leading-relaxed text-balance">
                待处理操作会写入服务器工作区，然后直接构建主站：<br />
                <span className="font-bold text-amber-500 break-all">{targetBlogPath}</span>
                <span className="text-xs opacity-80 text-slate-400 font-bold mt-2 block">构建期间后台将全局锁定，完成后自动切换主站版本。</span>
              </p>
              <div className="flex gap-3">
                ...
                <button onClick={executeServerPublish} className="flex-1 py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl text-xs font-black uppercase shadow-lg shadow-amber-500/30 transition-all">确认发布</button>
              </div>
            </motion.div>
          </div>
        )}
