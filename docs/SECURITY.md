...
| 评论账号与评论 | `/var/lib/xinghui-blog-admin/comments.sqlite3` | 仅服务用户可读写 |
| GitHub OAuth Secret | `/var/lib/xinghui-blog-admin/comments-oauth.json` | 600 |
...
- 后端阻止把当前 DeepSeek Key 写入公开站点配置；GitHub OAuth Secret 使用独立私有文件。
- 评论密码只保存 scrypt 哈希；会话使用 HttpOnly、Secure、SameSite Cookie 与 CSRF 令牌。
- 管理端密钥输入框关闭交叉自动填充。
- 构建期间后端拒绝写请求。
